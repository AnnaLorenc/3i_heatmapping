...
find_RR_days <- function(minInd,
  ...
  # days_you_want_RR_for allow to compute RR also for days not included in samples_on_days (a vector with)
  ...
  # samples_on_days is a dt with columns 'Assay_Date' and 'avail_WTs' (number of WT samples available on the given date)
{
      ...
      sapply(days_you_want_RR_for$Assay_Date, function(x) {
        ...
        if (nrow(for_cumsum_before) == 0) {
          ...
          if (for_cumsum_before[1, csb] < min_before) {
            ...
            start_before <-
              max(for_cumsum_before[csb >= min_before, Assay_Date])
          }
        }
  ...
  if (first_good_RR > last_good_RR) {
    ...
    dates_for_RR[dates_to_change_before, 2] <-
      dates_for_RR[first_good_RR, 2]
    dates_for_RR[dates_to_change_before, 3] <-
      dates_for_RR[first_good_RR, 3]
    dates_for_RR[dates_to_change_after, 2] <-
      dates_for_RR[last_good_RR, 2]
    dates_for_RR[dates_to_change_after, 3] <-
      dates_for_RR[last_good_RR, 3]
  }
  return(dates_for_RR)
}
...
compute_RR_simple <-
    ...
    #computes RR for the whole time range, for one parameter
  {
    ...
    if (split_by_sex) {
      WT_male <- WT_data[Sex == "Male",]
      WT_female <- WT_data[Sex == "Female",]
      miss_males <-
    ...
    perc_values <-
      get_percentiles_range_variable(WT_data[, par, with = F], percentile = percentile_cutoff)
    
    rr_dates <- sort(unique(c(WT_data$Assay_Date, days_without_WT)))
    class(rr_dates) <- "Date"
    RR <- data.frame(
      Assay_Date = rr_dates,
      upper = perc_values["upper"],
      lower  = perc_values["lower"],
      ind = sum(!is.na(WT_data[, par, with = F])),
  ...
  #helper function for compute_RR_befaft
{
  RR <- apply(days_for_RR, 1, function(x) {
    ...
    if (data_to_make_RR[, sum(!is.na(get(par)))] < minInd) {
      ...
      a <-
        get_percentiles_range_variable(data_to_make_RR[, par, with = F], percentile = percentile_cutoff)
      RR <-
        data.frame(
          Assay_Date = NA,
          upper = NA,
...
compute_RR_befaft <- function(WT_data,
  ...
  #Compute a time-window RR for the parameter par
{
  WT_data <-
    data.table(WT_data)[, c("Assay_Date", "Sex", par), with = F][order(Assay_Date)][!is.na((par))]
  
  if (!is.null(start)) {
    WT_data <- WT_data[Assay_Date >= start]
  }
  
  if (!is.null(end)) {
    WT_data <- WT_data[Assay_Date <= end]
  }
  ...
  WT_dt_sum <-
    WT_data[, .(avail_WTs = sum(!is.na(get(par)))), by = Assay_Date]
  # print(WT_dt_sum)
...
{
  if (is.null(WT_data)) {
    WT_data <- take_only_WTs(all_data_dt)
  }
  RR_full <- lapply(cols_for_RR, function(par) {
    print(par)
    days_without_WT = setdiff(all_data_dt[, Assay_Date], WT_data[, Assay_Date])
    class(days_without_WT) <- "Date"
    compute_RR_befaft(
      WT_data = WT_data,
      split_by_sex = TRUE,
...
check_if_mouse_outside_dt <-
    ...
    #Compare mouse values with RR and return info whether it is below/above/or within RR
  {
    ...
    #   print(pars)
    per_mouse <-
      apply(all_data_dt[, c("Assay_Date", "Sex", pars), with = F], 1,
            function(mouse) {
...
combine_outside <-
    ...
    #combines info from auto/premanual/postmanual 'outside info' in this order. Only for mice present in full manual (manual_fract, all mice done). Returns a dt with info about being outside of RR and another one with info about source of data used.
  {
    ...
    if (!all(cols_for_RR %in% colnames(result_pre))) {
      newcols <- setdiff(cols_for_RR, colnames(result_pre))
      add <-
        matrix(
    ...
    ra <-
      as.matrix(result_auto[order(Mouse)][, cols_for_RR, with = F])
    # print(dim(ra))
    rpre <-
      as.matrix(result_pre[order(Mouse)][, cols_for_RR, with = F])
    # print(dim(rpre))
    rpost <-
      as.matrix(result_post[order(Mouse)][, cols_for_RR, with = F])
    # print(dim(rpost))
...
fix_lack_of_entries <- function(dt,
                                column_name,
                                suffixes) {
  ...
  for (suff in suffixes) {
    ...
    df <-
      df[is.na(get(column_name)), (column_name) := ifelse(!is.na(get(paste0(
        column_name, suff
      ))), get(paste0(column_name, suff)), NA_character_), by = Mouse]
  }
  return(df)
}
...
tableh <- function(x)
  #summary: number of samples above/below RR (in -1/1 coding)
{
  return(list(
...
prepare_RR_whole_dtWrapper <- function(WT_data,
  ...
  {
  whole_dt_l <- lapply(cols_for_RR, function(single_par) {
    #removing outliers if suggested
    if (!is.null(outliers_dt)) {
      if (single_par %in% outliers_dt$par) {
        #   print(outliers_dt[par==single_par])
        WT_data_corrected <-
          WT_data[!(Assay_Date %in% outliers_dt[par == single_par, Assay_Date])]
      } else{
        WT_data_corrected = WT_data
      }
    } else{
      WT_data_corrected = WT_data
    }
    #   print(single_par)
    one_par = prepare_RR_whole (
      WT_data = WT_data_corrected,
  ...
  #this version takes ALL WT data as a dt. RR will be identical to all individuals of a given sex for a parameter (within split borders).
{
  ...
  if (!is.null(split_date)) {
    ...
    WT_data_after <-
      WT_data[Assay_Date >= split_date, ]#print("before split")
    ...
    return(list(
      res = rbind(
        prepare_RR_whole(
          ...
          min_ind = min_ind,
          single_par = single_par,
          assay_dates = assay_dates_before ,
          percentile_cutoff = percentile_cutoff
        )[["res"]],
        data.frame(
          date = split_date,
  ...
  if (split_by_sex) {
    #if RR is to be split by sex - divide data into male andfemale subset, run extract_data_for_local_RR separately and combine outputs
    ...
    res <-
      rbind(
        local_RR_f = data.table(local_RR_f$res, sex = "Female"),
        local_RR_m = data.table(local_RR_m$res, sex = "Male")
      )
    #   print(res)
  ...
  if (available_datapoints >= min_ind) {
    results <-
      get_percentiles_range_variable(WT_data[, get(single_par)], percentile=percentile_cutoff)
  } else{
  ...
  RRs_matrix <-
    matrix(rep(results, length(assay_dates)),
           nrow = length(assay_dates) ,
           byrow = TRUE)
...
identify_outliers <- function(dt,
                              coef_for_sd = 2.5,
                              colns = NULL) 
  #identifies outliers in parameters colns.
  #outlier is defined here as a day with daily median further than coef_for_sd from the overall median.
  #For the days without WT tested, a balanced representation of sexes and genotypes form tested mice is used.
  ...
  outliers <- lapply(colns, function(x) {
    a <-
      identify_outlying_days(take_only_WTs(dt),
                             par = x,
                             coef_for_sd = coef_for_sd)
    a[, wt_day := "YES"]
    
    print(x)
    b <-
      identify_outlying_days(
        dt = dt,
        assay_dates = unique(dt[!(Genotype %in% c("WT", "+/+", "+/Y")), Assay_Date]),
        par = x,
        coef_for_sd = coef_for_sd ,
        extraCare = TRUE
      )
...
balance_draw_one_parameter <- function(par, dt, assay_dates) 
  #to help with outlier detection on the dates when no WT were tested. Takes a balanced representation of Genotypes and sexes.
  {
...
find_perc_simple_dt <-
  function(WT_data,
    ...
    lisRes <- lapply(params, function(param) {
      ...
      if (!is.null(outliers_dt)) {
        if (param %in% outliers_dt$par) {
          WT_data_corrected <-
            WT_data[!(Assay_Date %in% outliers_dt[par == param, Assay_Date])]
        } else{
          WT_data_corrected = WT_data
        }
      } else{
    ...
    mergef <- function(a, b) {
      merge(a, b, by = cols_to_preserve, all = T)
    }
...
find_perc_simple_par <-
  function(WT_data,
    ...
    u = lapply(c("Male", "Female"), function(gender) {
      a = find_perc_simple_gender_par(WT_data,
                                      gender = gender,
                                      param,
                                      value = unlist(values[sex_vals == gender]))
...
find_perc_simple_gender_par <-
  function(WT_data, gender, param,  value) {
    # works for a vector to
    # print(EndRR)
    db_of_parameter <- unlist(WT_data[Sex == gender, param, with = F])
...
detect_hits_byparam_cp_1 <-
           ...
           cutoff_in = 5)
  {
    ...
    setkeyv(data_to_extract_without_WT,
            cols = c("Genotype", "Assay_Date", "Sex", param))
    data_param <-
      data_to_extract_without_WT[Genotype %in% genotype_to_pick, c("Colony_Prefix",
                                                                   "Genotype",
                                                                   "Gene_Name",
                                                                   "Assay_Date",
                                                                   "Sex",
    ...
    #  print(data_param)
    #  print(RR)
    if (nrow(data_param) < cutoff_in) {
      # <5????
      #  print(data_param)
      return(
        data.frame(
          param = param,
          data_param[1, .(Colony_Prefix, Gene_Name)] ,
          sum_below = NA,
          sum_above = NA,
          male_genotype = male_genotype,
          female_genotype = female_genotype,
          samples = nrow(data_param),
          comm = "not enough samples"
        )
      )
    }
...
match_by_factor_closest_n_eliminate_subset =
  #does the same as match_by_factor_closest_n, but excludes from matching a subset of individuals, sharing with the tested one a characteristics - for example, day of test, genotype etc.
  function(data_WT,
           data_one_ind_KO,
           parameter_to_match,
           closest_n = 70,
           byGend = F,
           param_for_exclusion = NULL) {
    #If tolerance_in_perc, it will take fraction tolerance_in_perc of the val_of_param_to_match
    val_of_param_to_match <-
      unlist(data_one_ind_KO[, parameter_to_match, with = F])
    ...
    if (byGend) {
      ...
      return(
        match_by_factor_closest_n_eliminate_subset(
          data_WT[Sex == unlist(data_one_ind_KO[, Sex])],
          data_one_ind_KO,
          parameter_to_match,
          closest_n = 70,
          byGend = F,
          param_for_exclusion = param_for_exclusion
        )[1:closest_n]
      )
    } else{
      if (!is.null(param_for_exclusion)) {
        value_of_param_for_exclusion <-
          unlist(data_one_ind_KO[, param_for_exclusion, with = F])
        #   print(value_of_param_for_exclusion)
        data_for_match <-
          data_WT[get(param_for_exclusion) != value_of_param_for_exclusion]
      } else{
...
match_and_call <- function(cols_for_RR,
                           parameter_to_match,
                           dss_data,
                           ...
                           upper_cutoff = 0.975,
                           byGend = TRUE)
  #To establish RR from animals matched  by sex (default) and another parameter. Used in DSS heatmapping, with weight and age as matching parameters.
  
{
  print(paste0(
    "Finding controls matching by sex and ",
    parameter_to_match,
    "..."
  ))
  matched_WT <-
    lapply(1:nrow(dss_data[Genotype != "WT"]), function (a) {
      list(
        tested = dss_data[Genotype != "WT"][a],
        wt = match_by_factor_closest_n_eliminate_subset(
          dss_data[Genotype == "WT"][Colony_Prefix == "CBSC"],
          dss_data[Genotype !=
                     "WT"][a],
          closest_n = 70,
  ...
  tests_sets <-
    lapply(cols_for_RR , function(parameter) {
      print(parameter)
      sapply(1:nrow(dss_data[Genotype != "WT"]), function(x) {
        locate_percentile <-
          ecdf(unlist(matched_WT[[x]]$wt[, parameter, with = F]))
        locate_percentile(matched_WT[[x]]$tested[, parameter, with = F])
      })
    })
  
  names(tests_sets) <- cols_for_RR
